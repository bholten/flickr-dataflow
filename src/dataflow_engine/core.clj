...
(def entries (keep (fn [node]
                     (when (= (:tag node) :entry)
                       ...
                       {:url (:href (:attrs (first (filter #(= "enclosure" (-> % :attrs :rel))
                                                           (:content node)))))
...
(defn writer [in out threads]
  (let [f (fn [{:keys [image size url] :as data}]
            (let [[width height] size
                  filename (str "images/" (munge (str url "_" width "_" height ".png")))]
...
(time
 (let [urls (chan 20)
   ...
   (resizer images resized 8 [[64 64] [128 128] [256 256]])
